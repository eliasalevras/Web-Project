...
if (isset($_SESSION['role']) && $_SESSION['role'] === "professor") {
    ...
    $sql = "SELECT ti.Thesis_ID, ti.Trimelous_Date, ti.Invitation_Status, ti.Thesis_Student_Number,
    CONCAT(p1.Professor_name,' ',p1.Professor_surname) as Professor
    FROM trimelous_invitation as ti
    INNER JOIN thesis ON ti.Thesis_ID = thesis.Thesis_ID
    INNER JOIN trimelis ON trimelis.Thesis_ID = ti.Thesis_ID
    LEFT  JOIN professor p1 ON p1.Professor_User_ID = ti.Professor_User_ID
...
<head>
    ...
    <title>Διδάσκοντες που έχουν προσκληθεί ως μέλος τριμελούς</title>
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
    ...
    <div class="card shadow-lg p-4">
        <h2 class="mb-4 text-center text-primary">Απαντήσεις Καθηγητών</h2>
 ...
 <div class="card shadow-sm">
  <div class="card-header"><strong>Απαντήσεις Καθηγητών</strong></div>
    ...
    <table class="table table-striped mb-0">
      <thead>
        <tr>
          <th>Thesis ID</th>
          <th>Student (AM)</th>
          <th><?= htmlspecialchars('Διδάσκων') ?></th>
          <th><?= htmlspecialchars('Απάντηση') ?></th>
          <th><?= htmlspecialchars('Ημερομηνία') ?></th>
        </tr>
      </thead>
      <tbody>
        <?php if (!$rows): ?>
        ...
        <?php else: ?>
          <?php foreach ($rows as $r): ?>
            <tr>
              ...
              <td><?= htmlspecialchars($r['Thesis_Student_Number']) ?></td>
              <td><?= htmlspecialchars($r['Professor'] ?? '—') ?></td>
              <td><?= htmlspecialchars($r['Invitation_Status'] ?? '—') ?></td>
              <td><?= htmlspecialchars($r['Trimelous_Date'] ?? '—') ?></td>
            </tr>
          <?php endforeach; ?>
        <?php endif; ?>
      </tbody>
