...
if (isset($_SESSION['role']) && $_SESSION['role'] === "professor") {
    ...
    if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['cancel_pending_thesis'])) {
        $student_am = $_POST['student_am'];
        $professor_id = $user['Professor_User_ID'];
        $sql = "SELECT Thesis_ID FROM thesis WHERE Thesis_Student = ? && Thesis_Epimelitis = ?";
        ...
        if ($row) {
            ...
            $sql2 = "UPDATE thesis SET Thesis_Status = 'cancel'
                     WHERE Thesis_Student = ? AND Thesis_Status = 'pending'";
            ...
            if ($affected > 0) {
                ...
                $message = '<div class="alert alert-success text-center">Δεν βρέθηκε υπό ανάθεση διατριβή για τον φοιτητή AM: ' .$student_am. '</div>.';

                // echo "No pending thesis found for student AM: " . $student_am;
            }
        } else {
            $message = '<div class="alert alert-success text-center"> Αυτός ο Φοιτητής δεν υπάρχει ή δεν είστε ο επιμελητής.</div>';
...
<head>
    ...
    <title>Ακύρωση "Υπό Ανάθεση" Διπλωματικής Εργασίας</title>
...
<script src="drophttps://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>


    <main class="col-md-8 col-lg-9 ms-sm-auto px-3 px-md-4 pt-4">
    <div class="card shadow-lg p-4">
        <h2 class="mb-4 text-center text-primary">Ακύρωση "Υπό Ανάθεση" Διπλωματικής Εργασίας</h2>
    ...
    ?>
        ...
        <form method="POST" action="cancel_pending_thesis.php" enctype="multipart/form-data">
            ...
            <div class="d-grid mt-4">
                <button type="submit" id="cancel_pending_thesis" name="cancel_pending_thesis" class="btn btn-primary btn-lg">
